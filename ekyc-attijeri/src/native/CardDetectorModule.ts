...
interface CardDetectorNativeModule {
  ...
  prepareBackScan(): Promise<boolean>;
...
class CardDetectorModule {
   ...
   * Called after initialize() on the key="back" mount.
   * Restores WAIT_BACK capture state and disables red validation.
   */
  async prepareBackScan(): Promise<boolean> {
    try {
      return await CardDetectorNative.prepareBackScan();
    } catch (error) {
      console.error('Failed to prepare back scan:', error);
      throw error;
    }
  }
