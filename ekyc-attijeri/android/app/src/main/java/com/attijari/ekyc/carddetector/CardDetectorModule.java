...
public class CardDetectorModule extends ReactContextBaseJavaModule {
     ...
     * Call after initialize() on the key="back" mount to restore WAIT_BACK state
     * and disable red validation so the back card can be detected.
     */
    @ReactMethod
    public void prepareBackScan(Promise promise) {
        try {
            CardDetectorJNI.nativePrepareBackScan();
            promise.resolve(true);
        } catch (Exception e) {
            promise.reject("PREPARE_BACK_ERROR", "Failed to prepare back scan: " + e.getMessage());
        }
    }
    ...
    public void autoCapture(String side, boolean layoutValid, Promise promise) {
        try {
            ...
            if (result.length >= 6) {
                map.putBoolean("isBlurry", result[4] > 0.5);
                map.putDouble("blurScore", result[5]);
            }
            if (result.length >= 7) {
                map.putBoolean("isLayoutInvalid", result[6] > 0.5);
            }
