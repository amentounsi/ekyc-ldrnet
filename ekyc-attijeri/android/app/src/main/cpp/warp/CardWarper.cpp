...
WarpResult CardWarper::warp(const cv::Mat& frame, 
    ...
    // Étape 4b: Unsharp masking to recover edge sharpness lost by bilinear interpolation.
    // Improves barcode bar edges and OCR text clarity.
    {
        cv::Mat blurred;
        cv::GaussianBlur(warped, blurred, cv::Size(0, 0), 1.5);
        cv::addWeighted(warped, 1.8, blurred, -0.8, 0.0, warped);
    }
