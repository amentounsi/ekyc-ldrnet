...
static bool g_hasWarpedImage = false;   // True only for the current frame's detection
static bool g_warpAvailableForCapture = false; // Persists across NONE frames until capture
...
static constexpr float BLUR_THRESHOLD = 22.f;  // 1280x720 input: 15 was too permissive; 22 rejects noticeably blurry frames while keeping acceptable ones
...
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeInit(JNIEnv*, jclass) {
    if (!g_detector) {
    ...
    if (!g_cinValidator) {
        ...
        LOGI("OfficialCINValidator initialised (layout gate enabled)");
    }
}

JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeRelease(JNIEnv*, jclass) {
    g_detector.reset();
    g_warper.reset();
    g_screenDetector.reset();
    g_cinValidator.reset();
    g_lastWarpedImage.release();
    g_hasWarpedImage = false;
    g_isScreenDisplay = false;
    g_screenConfidence = 0.f;
    LOGI("CardDetector, CardWarper, ScreenDetector and OfficialCINValidator released");
}

// ── Reset screen detection state (call when user wants to retry) ──
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeResetScreenDetection(JNIEnv*, jclass) {
    if (g_screenDetector) {
...
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeSetConfig(
    JNIEnv*, jclass,
...
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeSetOverlay(
    JNIEnv*, jclass,
    jboolean enabled,
...
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeSetScanMode(
    JNIEnv*, jclass,
    jint mode)
{
    if (!g_detector) { LOGE("not initialised"); return; }
...
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeDetectFromYUV(
    JNIEnv* env, jclass,
    ...
    jint rotationDegrees)
{
    ...
    if (result.isValid && result.corners.size() == 4 && g_warper) {
        ...
        if (warpResult.success) {
            ...
            g_warpAvailableForCapture = true; // Persists until capture is made
        ...
        // NOTE: do NOT reset g_blurScore / g_isBlurry here.
        // g_warpAvailableForCapture persists the last good warp for manual capture;
        // the blur state must stay tied to that warp so the blur gate remains active.
...
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeDetectFromBitmap(
...
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeDetectFromGrayscale(
    JNIEnv* env, jclass,
...
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeGetWarpedImage(JNIEnv* env, jclass) {
...
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeGetWarpedImageDimensions(JNIEnv* env, jclass) {
 ...
 */
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeClassifyCardSide(JNIEnv* env, jclass) {
 ...
 */
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeValidateLayout(
 ...
 */
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativePushPresenceFrame(JNIEnv*, jclass) {
 ...
 */
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeEvaluatePresence(JNIEnv* env, jclass) {
 ...
 */
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeResetPresence(JNIEnv*, jclass) {
    // BYPASS: Liveness disabled — do nothing
    if (!ENABLE_LIVENESS) {
...
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeGetCaptureState(JNIEnv*, jclass) {
 ...
 */
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeResetCaptureSequence(JNIEnv*, jclass) {
...
/**
 * Prepare native state for back scan.
 * Called when the back scan component mounts after a fresh initialize().
 * Sets captureState=WAIT_BACK and disables red validation so the back card
 * (which has no red flag corners) can be detected in Stage 6.
 */
JNIEXPORT void JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativePrepareBackScan(JNIEnv*, jclass) {
    LOGI(">>> nativePrepareBackScan: setting state=WAIT_BACK, redValidation=DISABLED");

    g_captureState = CaptureState::WAIT_BACK;
    g_hasCapturedFront = true;   // mark front as already captured
    g_hasCapturedBack  = false;

    // Clear any stale warp so the button press uses a fresh back frame
    g_warpAvailableForCapture = false;
    g_hasWarpedImage          = false;
    g_hasQuadCorners          = false;
    ...
    // Disable red validation — back card has no red flag corners
    if (g_detector) {
        CardDetection::DetectionConfig cfg = g_detector->getConfig();
        cfg.redValidationEnabled = false;
        g_detector->setConfig(cfg);
        g_detector->resetTemporalState();
        LOGI("Detector set to BACK mode (red validation DISABLED)");
    }
}
 ...
 *   [4] isBlurry       (1.0 if capture was blocked due to blur)
 *   [5] blurScore      (Laplacian variance; higher = sharper)
 *   [6] isLayoutInvalid (1.0 if capture was blocked due to layout mismatch)
 */
JNIEXPORT jfloatArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeAutoCapture(
        JNIEnv* env, jclass,
        jint detectedSide,
        jint layoutValid) {

    static constexpr int AUTOCAP_RESULT_LEN = 7;
    ...
    // ── BLUR GATE: Block capture if last warp is too blurry ───────────
    if (g_warpAvailableForCapture && g_isBlurry) {
        LOGI(">>> AUTO-CAPTURE: BLOCKED — blurry (score=%.1f < threshold=%.1f)",
             g_blurScore, BLUR_THRESHOLD);
        data[0] = 0.f;
        data[1] = static_cast<float>(static_cast<int>(g_captureState));
        data[2] = g_hasCapturedFront ? 1.f : 0.f;
        data[3] = g_hasCapturedBack  ? 1.f : 0.f;
        data[4] = 1.f; // isBlurry = true
        data[5] = g_blurScore;
        data[6] = 0.f;
        env->SetFloatArrayRegion(jresult, 0, AUTOCAP_RESULT_LEN, data);
        return jresult;
    ...
    // ── LAYOUT GATE: Block capture if warped image fails CIN layout validation ──
    // Runs OfficialCINValidator on g_lastWarpedImage (1000×630 grayscale).
    // FRONT scan: validates flag zone, photo zone, header text, ID number zone.
    // BACK  scan: validates fingerprint zone, barcode strip, stamp, text block.
    if (g_warpAvailableForCapture && !g_lastWarpedImage.empty() && g_cinValidator) {
        bool layoutPassed = false;
        float layoutScore = 0.f;

        if (g_captureState == CaptureState::WAIT_FRONT) {
            auto layoutResult = g_cinValidator->validateFront(g_lastWarpedImage);
            layoutPassed = layoutResult.valid;
            layoutScore  = layoutResult.score;
            LOGI(">>> LAYOUT-GATE [FRONT]: valid=%d score=%.2f (flag=%.2f photo=%.2f header=%.2f id=%.2f)",
                 layoutPassed, layoutScore,
                 layoutResult.flagScore, layoutResult.photoScore,
                 layoutResult.headerScore, layoutResult.idNumberScore);
        } else if (g_captureState == CaptureState::WAIT_BACK) {
            auto layoutResult = g_cinValidator->validateBack(g_lastWarpedImage);
            layoutPassed = layoutResult.valid;
            layoutScore  = layoutResult.score;
            LOGI(">>> LAYOUT-GATE [BACK]: valid=%d score=%.2f (fp=%.2f barcode=%.2f stamp=%.2f text=%.2f)",
                 layoutPassed, layoutScore,
                 layoutResult.fingerprintScore, layoutResult.barcodeScore,
                 layoutResult.stampScore, layoutResult.textScore);
        } else {
            layoutPassed = true; // FINISHED state — no gate needed
        }

        if (!layoutPassed) {
            LOGI(">>> AUTO-CAPTURE: BLOCKED — layout invalid (score=%.2f)", layoutScore);
            data[0] = 0.f;
            data[1] = static_cast<float>(static_cast<int>(g_captureState));
            data[2] = g_hasCapturedFront ? 1.f : 0.f;
            data[3] = g_hasCapturedBack  ? 1.f : 0.f;
            data[4] = 0.f;
            data[5] = g_blurScore;
            data[6] = 1.f; // isLayoutInvalid = true
            env->SetFloatArrayRegion(jresult, 0, AUTOCAP_RESULT_LEN, data);
            return jresult;
        }
    }

    // Capture from last successful warp.
    // frames where detection returns NONE, so the button press doesn't have to be
    // perfectly in sync with a detection frame.
    if (g_warpAvailableForCapture && !g_lastWarpedImage.empty()) {

        // STATE: WAIT_FRONT — capture front side
        if (g_captureState == CaptureState::WAIT_FRONT) {
            // Capture front
            g_capturedFrontImage = g_lastWarpedImage.clone();
            g_hasCapturedFront = true;
            captured = true;
            g_warpAvailableForCapture = false; // Consumed — wait for fresh back warp

            LOGI(">>> AUTO-CAPTURE: FRONT captured! blurScore=%.1f Transitioning to WAIT_BACK",
                 g_blurScore);
        ...
        // STATE: WAIT_BACK — capture back side
        else if (g_captureState == CaptureState::WAIT_BACK) {
            // Capture back
            ...
            LOGI(">>> AUTO-CAPTURE: BACK captured! blurScore=%.1f Transitioning to FINISHED",
                 g_blurScore);
    ...
    data[4] = 0.f;
    data[5] = g_blurScore;
    data[6] = 0.f; // isLayoutInvalid = false (capture succeeded or no warp available)
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeGetCapturedFront(JNIEnv* env, jclass) {
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_com_attijari_ekyc_carddetector_CardDetectorJNI_nativeGetCapturedBack(JNIEnv* env, jclass) {
