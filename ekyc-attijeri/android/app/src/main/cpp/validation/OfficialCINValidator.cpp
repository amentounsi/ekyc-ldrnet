...
float OfficialCINValidator::evaluateFlagZone(const cv::Mat& gray) {
    ...
    // ── Luminance gate: the FLAG zone (top-right of warped card) contains the
    // national coat-of-arms emblem — gold/bronze detail with dark outlines.
    // After CLAHE, real CIN emblem: mean ~140-200, stddev ~20-45 (detailed).
    // Student card / non-CIN: blank white here → mean ~210-245, stddev < 18 (uniform).
    // Reject only when BOTH bright AND featureless (uniform blank area).
    if (mean > 210.f && stddev < 18.f) {
        LOGI("FRONT Z1-Flag: REJECTED — blank zone (mean=%.1f > 210 && stddev=%.1f < 18)", mean, stddev);
        return 0.f;
    }
    ...
    LOGI("FRONT Z1-Flag: mean=%.1f stddev=%.1f edgeDens=%.4f → score=%.3f",
         mean, stddev, edgeDensity, score);
...
float OfficialCINValidator::evaluateFingerprintZone(const cv::Mat& gray) {
    ...
    // ── Isotropy check: fingerprint ridges curve in ALL directions.
    // Text is strongly anisotropic (horizontal >> vertical edges).
    float hEdge = computeEdgeDensitySobelX(roi);  // horizontal edges
    float vEdge = computeEdgeDensitySobelY(roi);  // vertical edges
    float isotropyRatio = 0.f;
    if (hEdge > 0.002f && vEdge > 0.002f) {
        isotropyRatio = std::min(hEdge, vEdge) / std::max(hEdge, vEdge);
    }
    float isotropyScore = clamp01(isotropyRatio / 0.55f);

    // Hard gate 1: anisotropic → text, not fingerprint
    if (isotropyRatio < 0.25f) {
        LOGI("BACK Z1-Fingerprint: REJECTED — isotropy=%.3f (text-like)", isotropyRatio);
        return 0.f;
    }

    // ── Local texture coverage: fingerprint ridges fill every corner of the ROI.
    // Divide into 8×8 blocks; count fraction with stddev > 8.0.
    // Fingerprint: >55% of blocks are highly textured (ridges everywhere).
    // Table grid / text: <40% of blocks are textured (mostly blank cell interiors).
    {
        const int bsz = 8;
        int total = 0, rich = 0;
        for (int r = 0; r + bsz <= roi.rows; r += bsz) {
            for (int c = 0; c + bsz <= roi.cols; c += bsz) {
                cv::Scalar bm, bs;
                cv::meanStdDev(roi(cv::Rect(c, r, bsz, bsz)), bm, bs);
                ++total;
                if (bs[0] > 8.0) ++rich;
            }
        }
        float textureCoverage = (total > 0)
            ? static_cast<float>(rich) / static_cast<float>(total) : 0.f;
        LOGI("BACK Z1-Fingerprint: texCoverage=%.3f (%d/%d blocks)",
             textureCoverage, rich, total);
        // Hard gate 2: low texture coverage → not a fingerprint
        if (textureCoverage < 0.45f) {
            LOGI("BACK Z1-Fingerprint: REJECTED — textureCoverage=%.3f < 0.45",
                 textureCoverage);
            return 0.f;
        }
    }
    ...
    float score = stdScore * 0.35f + edgeScore * 0.35f + isotropyScore * 0.30f;

    LOGI("BACK Z1-Fingerprint: stddev=%.1f edgeDens=%.4f hEdge=%.4f vEdge=%.4f isotropy=%.3f → score=%.3f",
         stddev, edgeDensity, hEdge, vEdge, isotropyRatio, score);
    return clamp01(score);
}
...
float OfficialCINValidator::evaluateBarcodeZone(const cv::Mat& gray) {
    ...
    // ── Hard gate 1: Otsu dark-pixel fraction.
    // A real barcode fills the strip with dense bars → 30-65% dark pixels.
    // Sparse alphanumeric text ("HM No 061765...") → only 5-18% dark pixels.
    {
        cv::Mat binarized;
        cv::threshold(roi, binarized, 0, 255, cv::THRESH_BINARY_INV | cv::THRESH_OTSU);
        float darkFraction = static_cast<float>(cv::countNonZero(binarized))
                           / static_cast<float>(binarized.total());
        LOGI("BACK Z2-Barcode: darkFrac=%.3f", darkFraction);
        if (darkFraction < 0.28f) {
            LOGI("BACK Z2-Barcode: REJECTED — darkFrac=%.3f < 0.28 (no barcode density)",
                 darkFraction);
            return 0.f;
        }
    }

    // ── Hard gate 2: vertical edge density.
    if (vEdgeDensity < CINLayoutConfig::BC_VEDGE_DENSITY_MIN) {
        LOGI("BACK Z2-Barcode: REJECTED — vEdge=%.4f < min=%.4f (no barcode bars)",
             vEdgeDensity, CINLayoutConfig::BC_VEDGE_DENSITY_MIN);
        return 0.f;
    }

    // Column projection — alternating barcode bars produce VERY high variance (600-2000).
    // Printed text "HM No 0617..." → ~50-200 variance.
    float colProjVar = computeColumnProjectionVariance(roi);

    float edgeScore = clamp01(vEdgeDensity / (CINLayoutConfig::BC_VEDGE_DENSITY_MIN * 2.5f));
    float projScore = clamp01(colProjVar / 600.f);
...
FrontLayoutResult OfficialCINValidator::validateFront(const cv::Mat& warpedImg) {
    ...
    // ── Per-zone minimum floors (hard reject if critical zones too weak) ──
    if (result.photoScore < CINLayoutConfig::FRONT_PHOTO_FLOOR) {
        LOGI("FRONT FLOOR REJECT: photoScore=%.3f < %.2f",
             result.photoScore, CINLayoutConfig::FRONT_PHOTO_FLOOR);
        result.valid = false;
        result.score = 0.f;
        return result;
    }
    if (result.flagScore < CINLayoutConfig::FRONT_FLAG_FLOOR) {
        LOGI("FRONT FLOOR REJECT: flagScore=%.3f < %.2f",
             result.flagScore, CINLayoutConfig::FRONT_FLAG_FLOOR);
        result.valid = false;
        result.score = 0.f;
        return result;
    }
